...
def run_with_context(app, func):
    """
    Ejecuta una función dentro del contexto de Flask
    """
...
def check_overdue_orders():
    """
    OTs correctivas asignadas hace más de X días sin iniciar
    """
    ...
    for order in overdue_orders:
        ...
        if rule and rule.escalation_hours and rule.escalation_target_role:
            ...
            escalation_time = order.assigned_at + timedelta(
            ...
            if datetime.utcnow() >= escalation_time:
                ...
                supervisors = User.query.filter_by(
                    role=rule.escalation_target_role
                ).all()
...
def check_low_life_equipment():
    ...
    threshold_percent = (
        rule.threshold_value if rule and rule.threshold_value else 10
    )
    ...
    equipments = Equipment.query.filter(
        ...
        (
            Equipment.estimated_life_hours -
...
def check_preventive_tasks():
    """
    Verifica tareas preventivas próximas a vencer o vencidas
    """

    today = datetime.utcnow().date()

    # Regla próximo a vencer
    ...
    for schedule in schedules:
        ...
        days_left = (
            schedule.next_due_date.date() - today
        ).days
        ...
        if due_soon_rule and due_soon_rule.threshold_value:
            ...
            threshold_days = int(
                due_soon_rule.threshold_value
            )

            if 0 < days_left <= threshold_days:

                target_roles = (
                    due_soon_rule.target_roles.split(',')
                    if due_soon_rule.target_roles
                    else ['tecnico', 'supervisor']
                )
        ...
        if overdue_rule and days_left < 0:
            ...
            target_roles = (
                overdue_rule.target_roles.split(',')
                if overdue_rule.target_roles
