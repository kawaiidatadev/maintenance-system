...
def create_order():
    if request.method == 'POST':
        ...
        # Manejar equipment_id: convertir string vacío a None
        ...
        if equipment_option == 'existing' and equipment_id:
            ...
            # Equipo no registrado o no seleccionado
            work_order = WorkOrder(
                number=number,
                equipment_id=None,
                problem_description=request.form.get('problem_description'),
...
def edit_order(id):
    ...
    if request.method == 'POST':
        ...
        order.assigned_to_id = int(assigned_to) if assigned_to and assigned_to != '' else None
        if order.assigned_to_id and order.status == 'open':
...
def start_order(id):
    order = WorkOrder.query.get_or_404(id)
    if order.assigned_to_id != current_user.id and current_user.role not in ['admin', 'supervisor']:
...
def complete_order(id):
    ...
    if request.method == 'POST':
        ...
        order.labor_hours = float(request.form.get('labor_hours') or 0)
